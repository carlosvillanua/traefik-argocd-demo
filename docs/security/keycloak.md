...
```
kubectl port-forward svc/keycloak-service --namespace traefik 7000 &
```
...
```
kubectl get secrets -n traefik keycloak-initial-admin -o jsonpath="{.data.username}" | base64 -d
kubectl get secrets -n traefik keycloak-initial-admin -o jsonpath="{.data.password}" | base64 -d
```
